...
for (ii in seq(length(MD_list))){
  ...
  Corr_Result = read.csv(paste(temp_Save_path, 'Step3_DTI_Corr_Result_X_group.csv', sep = '/'))
  ...
  Corr_Result$Region = sub('\\.', ' ', Corr_Result$Region)
  ...
  ggsave(paste(temp_Save_path, paste('DTI_Result_X_group of ', MD, ".pdf", sep = ""), sep = '/'), width = width_list[ii], height = 10)
