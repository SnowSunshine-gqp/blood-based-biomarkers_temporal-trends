...
for (MD in c("AX", "BIP", "DEP", "SCH")){
  ...
  Image_Data = read.csv(paste(Project_path, 'Data/Brain_Corr/Step2', MD, 'Step2_DKT.csv', sep = '/'))
  Regions = colnames(Image_Data)[22:length(colnames(Image_Data))]
  Region_name = data.frame(ori = Regions)
  ...
  write.csv(Corr_Result_X_group, paste(temp_Save_path, 'Step3_DKT_Corr_Result_X_group.csv', sep = '/'), row.names = T)
